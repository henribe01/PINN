...
class PendulumMLP(nn.Module):
    def __init__(self):
        ...
        # Input is time, initial angle and angular velocity
    ...
    def train_model(self, t, true_angles, t_physics, t_eval, num_epochs=1000, learning_rate=0.001, lambda_physical = 0.1, writer=None):
        ...
        with Bar('Training', max=num_epochs) as bar:
            for epoch in range(num_epochs):
                ...
                physical_residual = domega_dt + (9.81 / self.L) * torch.sin(predicted_physics)
...
if __name__ == "__main__":
   ...
   # Generate the data
    n_pendulums = 100
    true_angles, training_data = generate_data(n_pendulums, theta_span, omega_span, t_span, t_eval, L, g, noise_std=0.1)
    ...
    loss_history, L_history = model.train_model(t_train, true_angles_train, t_physics, t_eval_tensor, num_epochs=num_epochs, learning_rate=0.001, lambda_physical=0.1, writer=writer)
    writer.close()
